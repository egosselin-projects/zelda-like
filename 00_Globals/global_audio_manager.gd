...
func _ready() -> void:
	# Ignorer la pause induite par le changement de niveau pour éviter les coupures
	# de la musique
	process_mode = Node.PROCESS_MODE_ALWAYS
	
	for mp_index in music_audio_player_count:
...
func play_music(_audio: AudioStream) -> void:
	# continuer la lecture d'une musique identique
	...
	var old_player: AudioStreamPlayer = music_players[1]
	if current_music_player == 1:
