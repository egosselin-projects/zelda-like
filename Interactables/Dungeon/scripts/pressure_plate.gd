...
var bodies: int = 0 # compte des objets présents sur la zone de colision
...
func check_is_activated() -> void:
	if bodies > 0 and is_active == false:
		is_active = true
		# Bascule sur le mode appuyé (selection du sprite sur la sprite sheet)
		sprite.region_rect.position.x = off_rect.position.x - 32
		play_audio(audio_activate)
		activated.emit()
	elif bodies <= 0 and is_active == true:
		is_active = false
		# Bascule sur le mode relaché (selection du sprite sur la sprite sheet)
